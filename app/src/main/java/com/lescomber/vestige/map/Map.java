...
public class Map {
	...
	private static final int MAX_WAVE_GAP = 4000;    // Max time between waves (in ms)
	...
	public static final float PLAYER_HALF_WIDTH = 24.5f;
	public static final float PLAYER_HALF_HEIGHT = 19.5f;
	...
	public void setPortalPoint(float x, float y) {
		portal.offsetTo(x, y);
	}
	...
	public boolean isLevelComplete(Player player) {
		return (portal.isReady() && player.overlaps(portal));
	}
	...
	public int getWaveNum() {
		return waveCount - upcomingWaves.size();
	}
	...
	public boolean allSpawned() {
		return (currentWave.isFinished() && upcomingWaves.isEmpty()) && enemiesBuffer.isEmpty();
	}
