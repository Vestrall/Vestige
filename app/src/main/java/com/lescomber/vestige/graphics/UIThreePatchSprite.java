...
public class UIThreePatchSprite extends ThreePatchSprite {
	...
	/**
	 * Note: we don't bother calling the SpriteManager's setLayerHeight here because it would have no effect for the UI layer because the UI layer
	 * is not sorted by layerHeight. Any calls to UIThreePatchSprite's setLayerHeight must be made prior to setting its visibility to true in order
	 * to have any effect
	 */
	@Override
