		...
		if (removeMe != null && removeMe.getSprite().isVisible()) {
			if (addMe != null) {
				...
				if (addMe instanceof ThreePatchSprite) {
					SpriteManager.getInstance().replaceSprite(removeMe.getIndex(), addMe.getSprite().getInfo(), ((ThreePatchSprite) addMe)
							.getLeftTemplate(), ((ThreePatchSprite) addMe).getRightTemplate());
				} else
			...
			if (addMe != null) {
				...
				final int index;
				if (addMe instanceof ThreePatchSprite) {
					index = SpriteManager.getInstance()
							.newThreePatchSprite(addMe.getSprite().getInfo(), ((ThreePatchSprite) addMe).getLeftTemplate(), ((ThreePatchSprite)
									addMe)
									.getRightTemplate());
				} else
