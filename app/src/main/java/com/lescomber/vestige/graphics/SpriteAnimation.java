...
public class SpriteAnimation implements Image
{
	...
	// Remaining duration (in ms) for the entire animation. Normally an animation finishes whenever it completes all its sequences
	//but if duration is set (to a value >= 0) then the animation will stop abruptly when duration drops below 0
	...
	// Sequence limit refers to the number of times this animation should go through a full sequence before before being
	//stopped. Negative number means no limit
	...
	// Returns true if the animation finished its last sequence this frame
	...
	public void setSequenceDuration(int duration) { setFrameTime(Math.round(duration / frames.size())); }
	public void setFrameTime(int frameTime) { this.frameTime = frameTime; }
	public void setHoldLastFrame(boolean holdLastFrame) { this.holdLastFrame = holdLastFrame; }
	...
	@Override public int getIndex() { return curSprite.getIndex(); }
	@Override public float getX() { return sharedInfo.x; }
	@Override public float getY() { return sharedInfo.y; }
	@Override public float getDirection() { return sharedInfo.direction; }
