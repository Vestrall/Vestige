...
public class Text {
	...
	public Text(TextStyle style, String text, float swingX, float swingY, Alignment alignment, boolean isVisible) {
		...
		this.text = text;
		this.swingX = swingX;
		this.swingY = swingY - 3;    // Note: Hack to move text up slightly as it appears to be lower than expected
	...
	public void setVisible(boolean isVisible) {
		if (!this.isVisible && isVisible) {
			...
			index = TextManager
					.newString(style.getType(), text, centerX, centerY, (float) Math.toDegrees(direction), color[0], color[1], color[2], color[3]);
