...
public class Sprite implements Image {
	...
	/**
	 * This constructor is essentially just a wrapper to place a Sprite around a SpriteInfo. This Sprite will reference the parameter info rather
	 * than making a copy
	 */
	...
	public void setTexWidth(float percentage) {
		...
		// Calculate xOffset required to keep left edge of the image in place while we shrink the right side. dx = (difference in width) / 2
	...
	public float getWidth() {
		return info.widthScale * info.template.getWidth();
	}

	public float getHeight() {
		return info.heightScale * info.template.getHeight();
	}

	public float getWidthScale() {
		return info.widthScale;
	}

	public float getHeightScale() {
		return info.heightScale;
	}
