...
public class TextManager
{
	...
	// CGLTexts need to be created and load()'ed by the Renderer thread. However, we would like our game logic thread
	//to be deciding which CGLTexts to create. Therefore, when the game logic thread would like a new TextStyle
	//created, it must queue up the corresponding info for the creation of its CGLText, which, in this method, will
	//be initialized by the Renderer thread before any drawing takes place
	...
	public synchronized static float measureText(int list, int type, String text)
	{
		...
		{
			try { TextManager.class.wait(); } catch (final InterruptedException ie) { }
		}
	...
	public static int getBuildListNum() { return buildList; }
